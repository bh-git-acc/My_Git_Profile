...
//Search with invalid format
			...
			searchbox.sendKeys("!@#$%RDThdyhyjnhbsveaawdertyujkumjhngbfvcdwxs");
			driver.findElement(By.id("nav-search-submit-button")).click();
			WebElement result = driver.findElement(
					By.xpath("//span[contains(text(),\"Check each product page for other buying options.\")]"));

			if (result.isDisplayed()) {
				System.out.println("Tc-0042Success"+" Titlerecieved="+result.getText());
			} else {
				System.out.println("Failed- got a bug");
			}
      } catch (Exception e) {
			e.printStackTrace();
		} finally {
